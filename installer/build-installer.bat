...
setlocal
set "REPO=%~dp0.."
set "BIN_DIR=%REPO%\Builds\Win64\DisplayXR-test-2d-ui"
set "OUT_DIR=%~dp0"
...
"C:\Program Files (x86)\NSIS\makensis.exe" /DVERSION=%VERSION% /DVERSION_MAJOR=%VERSION_MAJOR% /DVERSION_MINOR=%VERSION_MINOR% /DVERSION_PATCH=%VERSION_PATCH% "/DBIN_DIR=%BIN_DIR%" "/DSOURCE_DIR=%REPO%" "/DOUTPUT_DIR=%OUT_DIR%" "%~dp0DisplayXRUnityTest2DUIInstaller.nsi" || exit /b 1

echo === DONE ===
echo Installer: %OUT_DIR%\DisplayXR-Unity-Test2DUI-Setup-%VERSION%.exe
